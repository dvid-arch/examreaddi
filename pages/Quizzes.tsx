...
const Quizzes: React.FC = () => {
    ...
    const availableYears = useMemo(() => {
        const years = new Set(pastPapersData.map(p => p.year));
    ...
    const mostRecentYearBySubject = useMemo(() => {
        ...
        subjects.forEach(subject => {
            const yearsForSubject = pastPapersData
    ...
    return (
        <div className="space-y-6">
            <Card>
                ...
                <p className="text-slate-600 mt-2">Choose your practice mode. Take a standard exam simulation or create a custom quiz tailored to your needs.</p>
                <div className="mt-4 flex border border-gray-200 rounded-lg p-1 bg-gray-50 max-w-sm">
                    <button
                        onClick={() => setPracticeMode('standard')}
                        className={`flex-1 py-2 px-4 rounded-md text-sm font-semibold transition-colors ${practiceMode === 'standard' ? 'bg-primary text-white shadow' : 'text-slate-600'}`}
                    >
                        Standard UTME Exam
                    </button>
                    <button
                        onClick={() => setPracticeMode('custom')}
                        className={`flex-1 py-2 px-4 rounded-md text-sm font-semibold transition-colors ${practiceMode === 'custom' ? 'bg-primary text-white shadow' : 'text-slate-600'}`}
                    >
                        Custom Practice
                    </button>
                </div>
            ...
            {practiceMode === 'standard' && (
                <Card>
                    <h2 className="text-xl font-bold text-slate-800">Choose Your Subjects ({selectedSubjects.length}/4)</h2>
                    <p className="text-slate-500 text-sm mb-4">English is compulsory. Please select 3 other subjects.</p>
                    ...
                    <div className="mt-6">
                        <label htmlFor="year-select" className="block text-xl font-bold text-slate-800 mb-2">Select Year</label>
                        <select
                            id="year-select"
                            value={String(standardSelectedYear)}
                            onChange={(e) => setStandardSelectedYear(e.target.value === 'random' ? 'random' : Number(e.target.value))}
                            className="w-full md:w-1/3 bg-gray-100 border-gray-200 border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary"
                        >
            ...
            {practiceMode === 'custom' && (
                <Card>
                    <form onSubmit={handleStartCustomPractice} className="space-y-6">
                         <h2 className="text-xl font-bold text-slate-800">Create a Custom Practice Session</h2>
                         
                         <div>
                            <h3 className="text-md font-semibold text-slate-700 mb-2">1. Select Subjects & Years</h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {subjects.map(subject => (
                                    <div key={subject} className={`p-3 border rounded-lg transition-colors ${customSelections[subject] ? 'bg-primary-light border-primary' : 'bg-white'}`}>
                                        ...
                                        {customSelections[subject] && (
                                            <div className="mt-2">
                                                <select
                                                    ...
                                                    className="w-full bg-gray-100 border-gray-200 border rounded-md px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-primary"
                                                >
                                                    {availableYears.map(year => (
                                                        <option key={year} value={year}>{year}</option>
                                                    ))}
                         ...
                         <div>
                            <label htmlFor="custom-count" className="block text-md font-semibold text-slate-700 mb-1">2. Number of Questions per Subject ({customQuestionCount})</label>
                             <input
                                id="custom-count"
                                type="range"
                                min="5"
