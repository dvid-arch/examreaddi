...
const preparePracticeQuestions = (selections: { subject: string, year: 'random' | number }[], questionsPerSubject: number): ChallengeQuestion[] => {
    ...
    sortedSelections.forEach(({ subject, year }) => {
        let papersForSubject = pastPapersData.filter(paper => paper.subject === subject);
...
const TakeExamination: React.FC = () => {
    ...
    }, [isFinished, questions, userAnswers, subjects, examTitle]);
    ...
    useEffect(() => {
        const { 
            ...
            questionsPerSubject,
            selections // new property for custom practice with individual years
        } = location.state || {};
        ...
        if (customQuestions && customQuestions.length > 0) {
            ...
            // New custom practice mode with individual years
            if (selections) {
            ...
            if (practiceSelections.length > 0) {
                preparedQuestions = preparePracticeQuestions(practiceSelections, numQuestions);
            }
        }
        ...
        if (preparedQuestions.length > 0) {
            ...
            setTimeLeft(preparedQuestions.length * 60);
             ...
             // Handle case where no questions could be prepared
             console.log("No questions prepared. Maybe redirect or show an error.");
        }
